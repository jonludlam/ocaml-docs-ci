...
module Make (Metadata : sig type t end) = struct
  ...
  (* Balanced-tree fold instead of a right-fold chain: a state change
     in one element re-evaluates only its O(log n) path to the root
     (and the unit/state cutoffs stop most of those), instead of every
     pair node between the element and the head of the list. *)
  let split_at i xs =
  ...
  let rec all = function
    ...
    | [x] -> x
    | xs ->
      let l, r = split_at (List.length xs / 2) xs in
      let+ () = all l
      and+ () = all r in
      ()
